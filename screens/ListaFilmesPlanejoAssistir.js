...
export default function ListaFilmesPlanejoAssistir() {
  const [lista, setLista] = useState([]);

  const carregarDados = async () => {
    const dados = await carregarLista('filmes_planejo');
    setLista(dados || []);
  };
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            await salvarLista('filmes_planejo', novaLista);
            setLista(novaLista);
          }
        }
      ]
  ...
  if (lista.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.text}>Nenhum filme na lista de planejamento.</Text>
      </View>
    );
  }
