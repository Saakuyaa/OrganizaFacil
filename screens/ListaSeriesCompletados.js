...
export default function ListaSeriesCompletados() {
  ...
  const carregarDados = async () => {
    const dados = await carregarLista('series_completados');
    setLista(dados || []);
  };
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            await salvarLista('series_completados', novaLista);
            setLista(novaLista);
          }
        }
      ]
  ...
  if (lista.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.text}>Nenhuma série completada ainda.</Text>
      </View>
    );
  }
