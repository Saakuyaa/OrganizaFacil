...
export default function ListaAnimesPlanejoAssistir() {
  ...
  const carregarDados = async () => {
    const dados = await carregarLista('animes_planejo');
    setLista(dados);
  };
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            await salvarLista('animes_planejo', novaLista);
            setLista(novaLista);
          }
        }
      ]
  ...
  if (lista.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.text}>Nenhum anime na lista de planejamento.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => abrirModal(item)} style={styles.item}>
            {item.imagem && (
