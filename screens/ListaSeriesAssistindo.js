...
export default function ListaSeriesAssistindo() {
  ...
  const carregarDados = async () => {
    const dados = await carregarLista('series_assistindo');
    setLista(dados || []);
  };
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            await salvarLista('series_assistindo', novaLista);
            setLista(novaLista);
          }
        }
      ]
  ...
  if (lista.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.text}>Nenhuma série assistindo no momento.</Text>
      </View>
    );
  }
