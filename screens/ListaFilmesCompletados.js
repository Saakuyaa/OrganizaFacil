...
export default function ListaFilmesCompletados() {
  ...
  const carregarDados = async () => {
    const dados = await carregarLista('filmes_completados');
    setLista(dados || []);
  };
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            await salvarLista('filmes_completados', novaLista);
            setLista(novaLista);
          }
        }
      ]
  ...
  if (lista.length === 0) {
    return (
      <View style={styles.container}>
        <Text style={styles.text}>Nenhum filme completado ainda.</Text>
      </View>
    );
  }
